...
# b) EDA tables  
```{r summary}
summary(crd_df) # Check the range of data
```

```{r glimpse}
glimpse(crd_df)  # Check the types of data
```

# c) Wrangling  
```{r crd_dfw}
# R treats data as numerical by default
# Change rep and trt as categorical
# If not, the model will produce linear regression instead of showing the means of each trt
crd_dfw <- crd_df %>%
...
```{r crd_dfw}
summary(crd_dfw)  # There is no statistical result about rep and trt since those are categorical now
```
...
# d) EDA plots  
```{r n boxplot}
# EDA (exploratory data analysis) plot. Visually exploring and summarizing the main characteristics.
# Simply checking your data before fitting models
ggplot(crd_dfw, aes(x = nrate_kgha,
                    y = yield_kgha,
                    color = nrate_kgha
                    )) +
  geom_boxplot() +
  geom_point() +  # show all the data point
  geom_jitter() +  # showing all the data but distributing horizontally when dots are too crowded
...
```{r nk boxplot}
# Check interaction effect by including both N and K in the plot
ggplot(crd_dfw, aes(x = nrate_kgha,
                    y = yield_kgha,
                    color = nrate_kgha
                    )) +
...
# Another way to express the interaction effect
ggplot(crd_dfw, aes(x = krate_kgha,
                    y = yield_kgha,
                    color = nrate_kgha
                    )) +
...
In R, the default contrast type is set-to-zero.(compare to a baseline, which is any random trt)

In research, we normally are interested in sum-to-zero contrasts.(compare to overall mean)
...
crd_mod <- lm(yield_kgha ~ nrate_kgha*krate_kgha,   # Shorter script but same function as above
              data = crd_dfw
              )

# Summary
summary(crd_mod)   # We don't need it actually
```
...
# f) ANOVA table  
The common functions `aov()` and `anova()` use type 1 SS, which is the wrong choice IF have unbalanced data. (when there are different sample sizes per trt; e.g., missing data)
...
**Just do not use aov() and anova(), but always use Anova()**
...
First, let's extract our model residuals, and also create studentized (standardized) residuals.  
```{r crd_resid}
...
- Residual independence (no pattern) : if trt were well randomized to experimental units
- Residual variance homogeneity (homoscedasticity) : Be careful of a Fan shape
- Residual normality (discuss this!) : QQ plot. checking residual's normality, not my data's
- Outlier detection (< -3 or > 3) :checking typing error or measurement errors
...
ggplot(crd_resid, aes(x = .fitted,
                      y = .studresid)) +
  geom_point(shape = 21,     # type of dots (e.g., 21 means a circle with a border)
             fill = "blue",  # fill color of dots
             size = 3,       # size of dots
             alpha = .7      # transparency of dots
             ) +
  geom_hline(yintercept = c(-3, 0, 3), color = "red") +   # Show a line -3 ~ 3 to check outlier
  geom_smooth()  # The blue line in the middle should be aligned to zero. No quadratic pattern
...
ggplot(crd_resid, aes(x=.fitted, y=.studresid))+
  ...
  geom_point(shape = 21,
             ...
             size = 3,
             alpha = .7)+
  geom_smooth()+  # The shaded gray area should not increase as fitted value increases (fan shape) even though the blue line is aligned to zero
  geom_hline(yintercept = c(-3,3), color = "red")+
  theme_bw() # Make academic/professional style of plot. White background and gray gridlines

```
Looks great! Next.  

## Residual normality  
- For this, we use the **quantile-quantile (QQ) plot** and **density plot (histogram)**.    
...
```{r}
ggplot(crd_resid, aes(x=.studresid)) +  # Y axis data is the "frequency (density)" of x data
  geom_density() +
  scale_x_continuous(breaks = c(-3,0,3), limits = c(-3,3))

# It is a little bit skewed to right and has heavy tail on both sides, but it is acceptable
```
...
emmeans : estimated marginal means (predicted values for a dependent variable across a regular grid of predictor combinations)
...
crd_means_nk <- emmeans(crd_mod, ~nrate_kgha|krate_kgha) # EMMs should be same as above (all)
...
crd_cld_all <- cld(crd_means_all,
                   reversed = T,         # Alphabetical order from high to low values
                   Letters = letters,    # We can change to number instead of alphabet
                   adjust = "none")    

# Tukey is default adjustment in R which is relatively more strict than no adjustment ("none")
...
crd_cld_selected <- crd_cld_all %>%
  as.data.frame() %>%
  mutate(letter = trimws(.group)) %>% # trimws: remove white space from the beginning and end
  mutate(trtname = paste0(nrate_kgha, "+", krate_kgha))
...
# If you are showing all 9 trt together in a plot, letters should also be the one from all combination comparison
# If you want to show comparison within either N or K (comparing 3, not 9), you should use facet function to separate plot into 3 parts.
